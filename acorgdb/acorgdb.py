...
class Antigen(Record):
    ...
    def sequence(self, gene: str = "HA") -> str:
        ...
        if has_own_seq:
            ...
            ) and not self.has_alt_parent_with_seq(gene):
            ...
            elif self.has_alt_parent_with_seq(gene) and has_subs:
                return mutate(self.alt_parent(gene).sequence(gene), subs)

            elif self.has_parent_with_seq(gene) and has_subs:
                return mutate(self.parent.sequence(gene), subs)

            elif not self.has_alt_parent_with_seq(
                gene
